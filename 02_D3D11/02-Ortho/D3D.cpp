...
#include <windows.h>		  // same as <stdio.h> library function, // includes around 3,50,000 API's. 
...
ID3D11InputLayout* gpD3D11InputLayout = NULL;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) //Winmain(main function which gives window(Hungarian Notation Used)), lpsz(Long Pointer To Zero-Terminated String ('\0')), hPrevInstance - for backward compatibility
{
	...
	//code
	/*if (fopen(&gpfile, "gszLogFileName", "w") != 0)
	{
	...
	if (FAILED(hr))
	{
		gpfile = fopen(gszLogFileName, "a+"); // a+ means append
...
HRESULT initialize(void)
{
	...
	dxgiSwapChainDesc.BufferCount = 1; //additional buffer is provided by D3D
	dxgiSwapChainDesc.BufferUsage = DXGI_USAGE_RENDER_TARGET_OUTPUT;
	dxgiSwapChainDesc.OutputWindow = ghwnd;
	dxgiSwapChainDesc.Windowed = TRUE;

	// Create D3D SwapChain, Device, Context, RTV(Render Target View)
	...
	{
		gpfile = fopen(gszLogFileName, "a+"); // a+ means append
		fprintf(gpfile, "D3D CreateDeviceAndSwapChain Failed! \n");
		fclose(gpfile);
		return(hr);
	}
	...
	{	
		...
		if (d3dDriverType == D3D_DRIVER_TYPE_HARDWARE)
			...
			fprintf(gpfile, "Driver Is Unknown\n");
		else
	...
	const char* vertexShaderSourceCode =
		"cbuffer" \ 
		"{" \
		"float 4\4 worldViewProjectionMatrix, pos"; \
		"ID3DBlob
		"}"; 		
...
{	
	...
	// step-2-resize the swapchain buffers according to the changed size
	gpIDXGISwapChain->ResizeBuffers(1, width, height, DXGI_FORMAT_R8G8B8A8_UNORM, 0);

	// step-3-create new rendertargetview
	// a - get the buffer from rtv(just like FBO)
	...
	hr = gpID3D11Device->CreateRenderTargetView(piD3D11Texture2D, NULL, &gpID3D11RenderTargetView); // mipmap is 0
	if (FAILED(hr))
	{
		gpfile = fopen(gszLogFileName, "a+"); // a+ means append
	...
	{
		gpfile = fopen(gszLogFileName, "a+"); // a+ means append
		fprintf(gpfile, "CreateRenderTargetView Succeeded!");
		fclose(gpfile);
	}
	...
	// c - set this new RenderTargetView In the Pipeline
	gpID3D11DeviceContext->OMSetRenderTargets(1, &gpID3D11RenderTargetView, NULL);

	// d - set the viewport (similar to OpenGL's ViewPort())
...
{
	// Code
	// clear the RTV using ClearColor
	gpID3D11DeviceContext->ClearRenderTargetView(gpID3D11RenderTargetView, clearColor);

	// do double buffering by presentinng the swapchain
