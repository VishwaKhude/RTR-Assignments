...
int main(void)
{
    ...
    // Add 10 to 'ptr' which is the address of 'num' and give the new address...
    ...
    // Add 10 to '*ptr' which is the value at address of 'num' (i.e., 'num i.e., 5) and give new value, without any change in address ...
    ...
    printf("Answer Of ++*ptr : %d\n", *ptr); //Brackets not necessary for pre - increment / pre-decrement

    // (RIGHT TO LEFT) Post-increment address ptr ... That is, address 'ptr' i.e., ptr is post-incremented (ptr++) and then the value ath the new address is displayed (*ptr++)...
    *ptr++; // Incorrect method of post-incrementing a value using pointer ...
    ...
    printf("Answer Of (*ptr)++ : %d\n", *ptr); //Brackets ARE neecessary for post-increment / post-decrement

    // (RIGHT TO LEFT) Post-increment value *ptr ... That is, value at address 'ptr' i.e., *ptr is post-incremented (*ptr)++
    ptr = &num;
    (*ptr)++; // Correct method of post-incrementing a value using pointer ...
       //*ptr is 6... at this statment *ptr remains 6 but aat next statement *ptr = 7 (post-increment)
    printf("Answer Of (*ptr)++ : %d\n\n", *ptr); //Brackets are necessary for post-increment / post-decrement
