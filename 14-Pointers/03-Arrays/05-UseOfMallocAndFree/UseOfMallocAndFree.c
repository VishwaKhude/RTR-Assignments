...
int main(void)
{
    ...
    int *ptr_iArray = NULL; //IT IS A GOOD DISCIPLINE TO INITIALIZE ANY POINTER WITH NULL TO PREVENT ANY GARBAGE VALUE GETTING INTO IT, THAT WAY, IT MAKES IT EASY TO CHECK FOR SUCCESS OR FAILURE OF MEMORY ALLOCATION LATER ON AFTER malloc()...
    ...
    // **** TO ALLOCATE SAID AMOUNT OF MEMEORY AND WILL RETURN THE INITIAL / STARTING / BASE ADDRESS OF THE ALLOCATED MEMORY, WHICH MUST BE CAPTURED IN A POINTER VARIABLE ****
    // **** USING THIS BASE ADDRESS, THE INTEGER ARRAY CAN BE ACCESSED AND USED ****

    ptr_iArray = (int *)malloc(sizeof(int) * intArrayLength);
    if (ptr_iArray == NULL) //if PTR_iArray IS STILL NULL, EVEN AFTER CALL TO  malloc(), IT MEANS malloc() in ptr_iArray...
    {
        printf("\n\n");
        printf("MEMORY ALLLOCATION FOR INTEGER ARRAY HAS FAILED !!! EXITTING NOW... \n\n");
        exit(0);
    }
