...
int initialize(void)
{
	...
	// changes regarding VAO and VBO for 
	...
	glBindVertexArray(vao_triangle);
	{
		...
		triangle_vertices.push_back(vmath::vec3(0.5f * sinf(vmath::radians(0.0f)), 0.5f * cosf(vmath::radians(0.0f)), 0.0f));
		triangle_vertices.push_back(vmath::vec3(0.5f * sinf(vmath::radians(120.0f)), 0.5f * cosf(vmath::radians(120.0f)), 0.0f));
		triangle_vertices.push_back(vmath::vec3(0.5f * sinf(vmath::radians(240.0f)), 0.5f * cosf(vmath::radians(240.0f)), 0.0f));
	...
	// changes regarding VAO and VBO for circle
...
void display(void)
{
	...
	glBindVertexArray(vao_triangle);
	glDrawArrays(GL_LINE_LOOP, 0, triangle_vertices.size());

	glBindVertexArray(vao_circle);
	glDrawArrays(GL_LINE_LOOP, 0, circle_vertices.size());

	// Unuse the Shader Progam Object
	glUseProgram(0);
