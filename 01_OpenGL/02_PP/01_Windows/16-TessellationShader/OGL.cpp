...
int initialize(void)
{
	...
	const GLchar* tesselationControlShaderSourceCode =
		...
		"uniformintuNumberOfSegments;"
		"uniformuNumberOfStrips;"
		"void main(void)"
		"{"
	...
	glShaderSource(tesselationControlShaderObject, 1, (const GLchar**)&tesselationControlShaderObject, NULL);
	...
	const GLchar* tesselationEvaluationShaderSourceCode =
		...
		"P1 * 3.0 * glTessCoord.x * (1 - glTessCoord.x) * (1 - glTessCoord.x) + P2 * 3.0 * (glTessCoord.x * glTessCoord.x * (1 - glTessCoord.x) + P3 * (glTessCoord.x * glTessCoord.x * glTessCoord.x);"
		"gl_position = uMVPMatrix * vec4(P, 1.0);"
		"}";

	GLuint tesselationEvaluationShaderObject = glCreateShader(GL_TESS_EVALUATION_SHADER);

	glShaderSource(tesselationEvaluationShaderObject, 1, (const GLchar**)&tesselationEvaluationShaderObject, NULL);
	...
	const GLfloat point_position[] =
	{
		...
		-0.5, 1.0, // 2nd
		0.5, -1.0, // 4th quadrant
		1.0, 1.0 // 1st quadrant
	};
...
void display(void)
{
	...
	// display changing number of line sgemnts in Window's title bar
	TCHAR str[255];
	wsprintf(str, TEXT("Vishwa Khude [numberOfLineSegemnts = %d]"), uiNumberOfLineSegments);
