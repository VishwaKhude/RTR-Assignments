...
mat4 orthographicProjectionMatrix;		//mat4 is in vmath.h
...
int initialize(void)
{
	...
	glClearColor(0.0f, 0.0f, 1.0f, 1.0f); // alpha(transperancy, opaqueness)
...
void resize(int width, int height)
{
	...
	if (width <= height)
	{
		orthographicProjectionMatrix = vmath::ortho(-100.0f,
			...
			100.0f * ((GLfloat)height / (GLfloat)width),
			-100.0f, 100.0f);							//left, right, bottom, top, far, near
	}
	else
	{
...
void display(void)
{
	...
	mat4 modelViewProjectionMatrix = orthographicProjectionMatrix * modelViewMatrix; //order of multiplication is v.imp in matrix multiplication
