...
#include "Smiley.h"		      // Our HeaderFile   
...
int initialize(void)
{
	...
	// Create Texture
	bResult = loadGLTexture(&texture_smiley, MAKEINTRESOURCE(MYSMILEYBITMAP));
	if (bResult == FALSE)
	{
...
void display(void)
{
	...
	glLoadIdentity();
	glTranslatef(0.0f, 0.0f, -4.0f); // -5.0f means towards the screen on z-axis (-)on z-axis
