...
GLuint texture_checkerboard = 0;  // GLuint (GraphicsLibrary unsigned integer)

unsigned int const checkimageWidth;
unsigned int const checkimageHeight;
...
int initialize(void)
{
	...
	glClearColor(0.75f, 0.75f, 0.75f, 1.0f); // alpha(transperancy, opaqueness)
...
void loadGLTexture(void)
{
	...
	// Code
	makeCheckImage();  //CheckImage gets filled 
	...
	glPixelStorei(GL_UNPACK_ALIGNMENT, 1);  // 1 Because the texture is Monochrome( only B&W)

	// Set Texture Parameters
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);  // S = X Axis in TEXTURES (wrap the texture horizontally)
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT); // T = Y Axis in TEXTURES (wrap the texture vertically)
...
void display(void)
{
	...
	glLoadIdentity();
	glTranslatef(0.0f, 0.0f, -3.6f); // -5.0f means towards the screen on z-axis (-)on z-axis
	...
	//2nd QUAD
	glTexCoord2f(1.0f, 1.0f);		//Right Top
	glVertex3f(2.4142f, 1.0f, -1.41421f);
