...
#include <windows.h>          // same as <stdio.h> library function
							  // includes around 3,50,000 API's 
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) //Winmain(main function which gives window(Hungarian Notation Used)), lpsz(Long Pointer To Zero-Terminated String ('\0')), hPrevInstance - for backward compatibility
{
	...
	// Create Window
	hwnd = CreateWindow(szAppName, // string to NULL terminated app name('\0') 
									 //Creates window in memory
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam) // UINT- Unsigned INTeger iMsg(integer Message) 
{
	...
	{
	case WM_CREATE :
		MessageBox(hwnd, TEXT("WM_CREATE Arrived"),
			TEXT("Message"), MB_OK);
		break;

	case WM_SIZE :
		MessageBox(hwnd, TEXT("WM_SIZE Arrived"),
			TEXT("Message"), MB_OK);
		break;
	...
	case WM_KEYDOWN :
	{
		...
		{
		case VK_ESCAPE :
			MessageBox(hwnd, TEXT("WM_KEYDOWN:VK_ESCAPE Arrived"),
				TEXT("Message"), MB_OK);
			break;
		}
	}
	...
	case WM_CHAR :
	{
		...
		{
		case 'F':
			MessageBox(hwnd, TEXT("WM_CHAR - F Key is Pressed"),
				TEXT("Message"), MB_OK);
				break;
		}
		break;
	...
	{
			MessageBox(hwnd, TEXT("	WM_DESTROY Arrived"),
			TEXT("Message"), MB_OK);
		PostQuitMessage(0);
		break;
	}
