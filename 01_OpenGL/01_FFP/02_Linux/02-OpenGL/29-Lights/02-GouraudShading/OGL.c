...
GLfloat lightAmbient[] = { 0.5f, 0.5f, 0.5f, 1.0f };  //grey ambient light
...
void initialise(void)
{
	...
	glLightfv(GL_LIGHT0, GL_AMBIENT, lightAmbient);  //glLightfv - graphicslibraryLight((float)-value)vectorarray
...
void draw(void)
{
	...
	glTranslatef(0.0f, 0.0f, -1.0f); // -3.0f means towards the screen on z-axis (-)on z-axis
	
	gluSphere(quadric, 0.2f, 50, 50); //gluSphere call internally creates all Normals for you
