...
void draw(void)
{
	...
	glTranslatef(-1.5f, 0.0f, -6.0f); // -3.0f means towards the screen on z-axis (-)on z-axis
	...
	glVertex3f(-1.0f, -1.0f, 1.0f); //left vertex
	glColor3f(0.0f, 0.0f, 1.0f);
	glVertex3f(1.0f, -1.0f, 1.0f); //right vertex
	...
	glTranslatef(1.5f, 0.0f, -6.0f); // -5.0f means towards the screen on z-axis (-)on z-axis
...
{
	//code
	pangle = pangle + 0.1f;  // or tangle += 1.0f
	if (pangle >= 360.0f)
	pangle = pangle - 360.0f;

	cangle = cangle - 0.1f;  // or tangle += 1.0f
	if (cangle >= 360.0f)
