...
int main(void)
{
    ...
    {
        if (i == 0) //First Letter Of Any One Sentence Must Be A CAPITAL LETTER
        ...
        else if ((chArray[i] == FULLSTOP || chArray[i] == COMMA || chArray[i] == EXCLAMATION || chArray[i] == QUESTION_MARK) && (chArray[i] != SPACE)) //First Letter Of Every Word Afer Punctuation Mark, In The Sentence Must Be A CAPITAL LETTER. Words Are Separated By Punctuations.
        {
            chArray_CapitalizedFirstLetterOfEveryWord[j] = chArray[i];
            ...
            //
            //SINCE, ALREADY TWO CHARACTERS (AT INDICES 'i' AND i + 1 HAVE BEEN CONSIDERED IN THIS else-if BLOCK...WE ARE EXTRA-INCREMENTING 'j' BY 2
            j = j + 2;
            i++; 
        }
...
char MyToUpper(char ch)
{
    ...
    // IF THIS DIFFEENCE IS SUBTRACTED FROM THE ASCII VALUE OF A LOWER LETTER, THE RESULTANT ASCII VALUE WILL BE THAT OF ITS UPPER CASE COUNTER-PART, HENCE, HELPING US TO FIND IT'S UPPER CASE LETTER !!!
