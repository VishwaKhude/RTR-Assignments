...
int main(int argc, char *argv[], char *envp[]) //envp -
{
    ...
    // *** DUE TO USE OF atoi(), ALL COMMAND LINE ARGUMENSTS OF TYPES OTHER THAN 'int' ARE IGNORED ***
    printf("\n\n");
    printf("Sum of All integer Command Line arguments Is : \n\n");
    for (i = 1; i < argc; i++) //loop starts from i = 1 because, i = 0 will result in 'argv[i]' = 'argv[0]' which is the name of the program iself i.e : 'CommandLineArgumentsApplication.exe'
    {
        num = atoi(argv[i]);
        sum = sum + num;
    }
