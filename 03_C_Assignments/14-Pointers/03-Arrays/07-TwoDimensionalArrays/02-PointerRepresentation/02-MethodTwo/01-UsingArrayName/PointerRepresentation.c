...
int main(void)
{
    ...
    // iArray[5][3] => iArray IS A 2D ARRAY HAVING 5 ROWS AND 3 COLUMNS. EACH OF THESE 5 ROWS IS A 1D INTEGERR ARRAY OF 3 INTEGERS ***
    ...
    // (iArray[1] + 2) => ADDRESS OF 2NS INTEGER FROM BASE ADDRESS OF 1ST ROW (iArray[1][2])
    ...
    // (*(iArray + 1) + 2) = (iArray[1] + 2) = ADDRESS OF 2ND ELEMENT FROM BASE ADDRESS OF 2ND ROW = (iArray[1] + 2) = (iArray[1][2])

    for (i = 0; i < NUM_ROWS; i++)
    {
        for (j = 0; j < NUM_COLUMNS; j++)
        *(*(iArray + i) + j) = (i + 1); // 'iArray[i]' Can Be Treated As 1D Array Using Pointers ...
    }
