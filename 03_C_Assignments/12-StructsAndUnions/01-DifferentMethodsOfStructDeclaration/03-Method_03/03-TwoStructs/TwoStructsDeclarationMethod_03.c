...
int main(void)
{
    ...
    if (point.x == 0 && point.y == 0) //If 'X' IS ZERO...OBVIOUSLY 'Y' IS THE NON-ZERO VALUE 
    {
    ...
    {
        point_properties.axis_location[0] = '\0'; // A Point Lying In ANY Of The 4 Quadrants Cannot BE Lying On Any Of THe C-ordinate Axes...

        if (point.x > 0 && point.y > 0)     // 'X' IS +ve NAD 'Y' IS +ve
            point_properties.quadrant = 1;

        else if (point.x < 0 && point.y > 0)     // 'X' IS -ve NAD 'Y' IS +ve
            point_properties.quadrant = 2;

        else if (point.x < 0 && point.y < 0)     // 'X' IS -ve NAD 'Y' IS -ve
            point_properties.quadrant = 3;

        else                                     // 'X' IS +ve NAD 'Y' IS -ve
