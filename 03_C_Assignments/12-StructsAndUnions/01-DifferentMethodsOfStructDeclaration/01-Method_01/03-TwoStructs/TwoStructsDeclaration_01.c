...
} point_properties; //declaring a single variable of type 'struct MyPointProperties' gobally...
...
int main(void)
{
    ...
    {
        if (point.x == 0) //If 'X' IS ZERO...OBVIOUSLY 'y' IS THE NO-ZERO VALUE
        {
        ...
        else if (point.y == 0) // If 'Y' IS ZERO... OBVIOUSLY 'X' IS THE NON-ZERO VALUE
        {
            ...
            point_properties.quadrant = 0; //A Point Lying On Any Of HTe Co-ordinate Axes Is NOT A Part Of ANY Quadrant...
            printf("The Point Lies On The %s Axis !!!\n\n", point_properties.axis_location);
        }
